"""
Board and game-state understanding scenarios.
...
SCENARIO_VP_CALCULATION = UnderstandingScenario(
    ...
    desired_answer=(
        "BLUE has 6 visible VP: 2 settlements (2 VP) + 1 city (2 VP) + Longest Road (2 VP)."
    ),
)
...
SCENARIO_DEV_CARD_LOGIC = UnderstandingScenario(
    ...
    desired_answer=(
        "Buy a development card (costs 1 Ore + 1 Sheep + 1 Wheat). "
        "RED is at 9 actual VP with 2 hidden VP cards already. "
        "No settlements can be built (no buildable nodes) and a city requires "
        "2 Wheat but RED only has 1. A dev card is the only path to 10 VP."
    ),
)
...
SCENARIO_LARGEST_ARMY = UnderstandingScenario(
    ...
    desired_answer=(
        "BLUE still holds Largest Army. Playing the Knight gives RED 3 played knights, "
        "tying BLUE — but a tie does NOT transfer Largest Army. The current holder "
        "keeps it; you need strictly MORE knights than the current holder to claim it."
    ),
)
...
SCENARIO_TRADE_EVALUATION = UnderstandingScenario(
    ...
    desired_answer=(
        "Before: Wood 1, Sheep 2, Wheat 1, Ore 0. "
        "After: Wood 0, Sheep 1, Wheat 1, Ore 1. "
        "The trade moves RED 1/3 of the way to the Ore needed, but still short "
        "1 Wheat and 2 Ore. It helps marginally but doesn't get RED there alone."
    ),
)
...
SCENARIO_DEFENSIVE_TRADE = UnderstandingScenario(
    ...
    desired_answer=(
        "Reject the trade. BLUE is at 9 VP — one resource away from potentially "
        "winning. Giving them Wheat (used for cities and dev cards) could hand them "
        "the win regardless of how good the offered Wood + Brick looks."
    ),
)
...
SCENARIO_INITIAL_PLACEMENT = UnderstandingScenario(
    ...
    desired_answer=(
        "Node 9 (Wood-8, Ore-10, Wheat-6) supports City/DevCard strategy — "
        "it produces Ore and Wheat, the two city/dev-card resources. "
        "Node 2 (Brick-8, Wood-8, Wheat-6) supports expansion with high-probability "
        "Wood and Brick. Node 9 is the stronger pick for a development-focused build."
    ),
)
...
SCENARIO_DISTANCE_RULE = UnderstandingScenario(
    ...
    desired_answer=(
        "The distance rule requires settlements to be at least 2 road lengths apart. "
        "Node 1 is adjacent (1 edge away) to both RED's settlements at Node 0 and "
        "Node 2, so placing there would violate the rule."
    ),
)
...
SCENARIO_ROAD_CUTTING = UnderstandingScenario(
    ...
    desired_answer=(
        "No. An enemy settlement on a node in the middle of a road chain breaks "
        "continuity at that point. BLUE's road would be split into (24,25)-(25,26) "
        "and (26,27) separately — the longest continuous segment would be 2, not 3."
    ),
)
...
SCENARIO_ROBBER_HEX_EVAL = UnderstandingScenario(
    ...
    desired_answer=(
        "Place on the highest-probability hex adjacent to BLUE's city (double production). "
        "A city produces 2 resources per roll, so blocking it maximises denial. "
        "Prefer dice numbers 6 or 8 (5/36 each) over lower-probability tiles."
    ),
)
...
SCENARIO_TRAPPED_SETTLEMENT = UnderstandingScenario(
    ...
    desired_answer=(
        "No. All three edges from Node 2 are occupied: RED owns (1,2), "
        "BLUE owns (2,3) and (2,9). There are no free edges to build a road from Node 2."
    ),
)
...
SCENARIO_COORDINATE_ADJACENCY = UnderstandingScenario(
    ...
    desired_answer=(
        "Edges (0,5) and (0,20). Node 0 has three adjacent edges: (0,1), (0,5), (0,20). "
        "RED already owns (0,1), so the two remaining free edges are the buildable ones."
    ),
)
...
SCENARIO_CONNECTING_ROADS = UnderstandingScenario(
    ...
    desired_answer=(
        "Yes — build on edge (2, 9). RED's segments are (0,1)-(1,2) and (9,10)-(10,11). "
        "Node 2 and node 9 are the endpoints where they meet. Connecting them gives "
        "a continuous chain of length 5, which would claim Longest Road."
    ),
)
...
SCENARIO_SPATIAL_BLOCKING = UnderstandingScenario(
    ...
    desired_answer=(
        "Build a road on edge (34, 33) or (34, 35). RED already controls node 34 "
        "via the road (13,34). BLUE needs to pass through node 34 to connect 32 and 37. "
        "Placing a road on either side of 34 blocks both approaches simultaneously."
    ),
)
...
SCENARIO_CONNECTING_ROADS_UNHINTED = UnderstandingScenario(
    ...
    desired_answer=(
        "Build on edge (2, 9) to connect the two separate road segments and form "
        "a continuous chain of length 5, claiming Longest Road (2 VP)."
    ),
)
...
SCENARIO_SPATIAL_BLOCKING_UNHINTED = UnderstandingScenario(
    ...
    desired_answer=(
        "Build on (34, 33) or (34, 35) to block BLUE's only path between their two "
        "settlements. RED controls node 34 and can seal off the corridor before BLUE "
        "can extend their road network through it."
    ),
)
...
SCENARIO_ROBBER_TARGETING = UnderstandingScenario(
    ...
    desired_answer=(
        "Place the robber on BLUE's highest-production hex (best dice number adjacent "
        "to a BLUE building), preferably a city hex for 2x denial. Steal from BLUE "
        "to slow them down — at 8 VP they are close to winning, so both denying "
        "income and stealing are important."
    ),
)
...
SCENARIO_LONGEST_ROAD_RACE = UnderstandingScenario(
    ...
    desired_answer=(
        "BLUE will almost certainly build 2 roads to reach road length 6, "
        "overtaking RED's current Longest Road of 5. This transfers the 2 VP "
        "bonus to BLUE and costs RED those VP. RED should consider blocking "
        "BLUE's likely road extension path."
    ),
)
